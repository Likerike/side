...
<section class="u-align-center u-clearfix u-section-8" id="carousel_af08">
  <div class="u-clearfix u-sheet u-sheet-1">
    <h1 class="u-custom-font u-font-oswald u-text u-text-default u-text-1">Заявления на ремонт</h1>
    <div class="u-expanded-width u-table u-table-responsive u-table-1">
      <table class="u-table-entity u-table-entity-1">
        ...
        <tbody class="u-align-center u-table-body">
          @foreach($repair as $key => $re)
          <tr style="height: 75px;">
            <td class=" u-border-1 u-border-grey-30 u-table-cell">{{ $key }}</td>
            <td class=" u-border-1 u-border-grey-30 u-table-cell">{{ $re->name }} {{ $re->surname }}</td>
            <td class=" u-border-1 u-border-grey-30 u-table-cell">{{ $re->created_at }}</td>
            <td class=" u-border-1 u-border-grey-30 u-table-cell">{{ $re->phone }}</td>
            <td class=" u-border-1 u-border-grey-30 u-table-cell">{{ $re->problem }}</td>
            <form action="{{ route('list-orders-for-repair-okey', $re->id) }}" method='post'>
                @csrf
                <td class="u-border-1 u-border-white u-table-cell "><button>Принять</button></td>
            </form>
            <form action="{{ route('list-orders-for-repair-no', $re->id) }}" method='post'>
                @csrf
