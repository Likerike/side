...
<section class="u-align-center u-clearfix u-section-8" id="carousel_af08">
  <div class="u-clearfix u-sheet u-sheet-1">
    <h1 class="u-custom-font u-font-oswald u-text u-text-default u-text-1">Заявления на настройку ОС</h1>
      ...
      <table class="u-table-entity u-table-entity-1">
        ...
        <tbody class="u-align-center u-table-body">
          ...
          <tr style="height: 75px;">
            ...
            <td class="u-border-1 u-border-grey-30 u-table-cell">{{ $st->name }} {{ $st->surname }}</td>
            <td class="u-border-1 u-border-grey-30 u-table-cell">{{ $st->created_at }}</td>
            <td class="u-border-1 u-border-grey-30 u-table-cell">{{ $st->phone }}</td>
            <td class="u-border-1 u-border-grey-30 u-table-cell">
              Вид услуги: {{ $st->service }}
            </td>
            <form action="{{ route('list-orders-for-setings-OS-okey', $st->id) }}" method='post'>
                @csrf
                <td class="u-border-1 u-border-white u-table-cell"><button>Принять</button></td>
            </form>
            <form action="{{ route('list-orders-for-setings-OS-no', $st->id) }}" method='post'>
                @csrf
